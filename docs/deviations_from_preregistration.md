...
**Manuscript title:** "Sixteen models, fewer than two voices: effective diversity in a language model ensemble on psychotherapeutic case formulation".

This is the third manuscript title. Two earlier ones were carried in draft: "Measuring when a language model ensemble has no single answer: effective diversity as a trigger for meaningful human oversight", and the same title with "candidate signal" in place of "trigger". The current title was adopted after a post-hoc analysis, reported in the Results, found that dissent does not track the interpretive openness for which the case bank was stratified; presenting the measure as a signal for oversight would overstate what the study supports.
